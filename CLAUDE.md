...
- **IDOR (crítico) — RESUELTO (jun 2026)**: las funciones Convex de datos ya NO se fían del `userId` del cliente; derivan la identidad de la sesión verificada con `requireUserId(ctx)` (`convex/authz.ts`). **Fase 1**: Better Auth emite JWT (plugin `jwt`, **RS256** — Convex Custom JWT no soporta EdDSA) y Convex lo valida (provider `customJwt` en `convex/auth.config.ts` con `applicationID`=issuer=`SITE_URL`, tabla `jwks` manejada por el adaptador, `ConvexProviderWithAuth`). **Fase 2**: todas las funciones públicas llamadas por el frontend usan `requireUserId` (arg `userId` queda `v.optional` e IGNORADO por compat — limpieza cosmética pendiente); añadidos checks de ownership que faltaban (`markNotificationRead`, `getStatementById`, `setTransactionExcluded`, `updateTransactionCategory`, `listTransactions` por `statementId`). **Patrón clave**: `/app` y `/onboarding` se envuelven en `ConvexAuthGate` (no renderizan hasta que Convex está autenticado → sin carrera "query antes del token"). **Funciones de doble uso** (web + API HTTP sin sesión) divididas en wrapper público (`requireUserId`) + `internalAction`/`internalQuery` con userId de confianza: `process.processStatement`/`processStatementInternal`, `insightsActions.generateMonthInsights`/`...Internal`; `processApiTransactions` pasó a interna; `http.ts` llama variantes internas (`listStatementsInternal`, `getStatementByIdInternal`, `getMonthInsightsInternal`, `getUserTaxonomyInternal`) con el userId de la API key. Verificado en prod: spoof de userId ajeno se ignora, sin token → "No autenticado", API pública sigue 401-sin-key. Detalle en **`SECURITY-IDOR-PLAN.md`**.
- **Pendiente menor**: quitar la query temporal `authz:whoami`, borrar usuarios de prueba `idor-*@example.com`, y (cosmético) quitar los args `userId` opcionales ya ignorados del backend + del frontend.
