...
Chat (Pro) en el `AiPanel` (columna derecha de `/app`) que responde preguntas libres sobre las finanzas del usuario.

- **`convex/assistantActions.ts:ask`** (`"use node"`, args `userId`/`question`/`history?`): comprueba Pro server-side (`getSubscriptionInternal` + `subscriptionIsPro`), carga `getAllTransactionsInternal` (excluye `excluded`), y construye un **resumen agregado compacto** con `buildContext` — NUNCA se envían transacciones en crudo a Claude. El resumen incluye: `porMes` (ingresos/gastos/ahorro, últimos 24 meses), `categoriasPorMes` (matriz categoría×mes para tendencias), `topGastosUnicos` (10 mayores gastos), `topComercios` (agrupados por `merchantPattern`), `totalSuscripciones`. Modelo `claude-sonnet-4-6`, `max_tokens: 700`, el resumen va en el `system` y la pregunta + historial en `messages`.
- **Historial**: el cliente pasa los últimos 8 turnos; la action recorta mensajes `assistant` iniciales para que la secuencia empiece por `user` (requisito de la API de Anthropic).
- **`src/components/app/AiPanel.tsx`**: `useAction(api.assistantActions.ask)`; estado `typing`, manejo de error inline, sugerencias de ejemplo. Sigue bloqueado para Free (tarjeta "Activar Pro").
