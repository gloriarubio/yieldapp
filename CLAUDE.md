...
- **IDOR (crítico) — Fase 1 hecha, Fase 2 pendiente**: las funciones Convex de datos aún se fían del `userId` del cliente. **Fase 1 (completa)**: Better Auth emite JWT (plugin `jwt`, RS256) y Convex lo valida → `ctx.auth.getUserIdentity()` funciona en prod (provider `customJwt` en `convex/auth.config.ts`, tabla `jwks`, `ConvexProviderWithAuth`, helper `requireUserId` en `convex/authz.ts`). **Fase 2 (pendiente)**: migrar las funciones a `requireUserId(ctx)` + checks de ownership y quitar el `userId` de los args. Detalle y aprendizajes (RS256 obligatorio, `applicationID`=aud, modelo `jwks`) en **`SECURITY-IDOR-PLAN.md`**. El adaptador Better Auth (`betterAuth.ts`, valida `CONVEX_ADAPTER_SECRET`) y la API HTTP (`http.ts`, API key) NO están afectados.
